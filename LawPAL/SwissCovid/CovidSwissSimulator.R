...
simulate_SEIR_forced <- function(
  ...
  for (t in 2:(T+1)) {
    ...
    reported_cases[t-1] <- rbinom(1, new_I, rtruncnorm(T_length,0,1,q,sigma_q))
...
sim <- simulate_SEIR_forced(T=109, N = 8570000 , beta = samples$beta[i],rho = samples$rho[i], gamma = samples$gamma[i],I0 = samples$i0[i],
                            E0 = samples$e0[i],R0 = 0, q = samples$q[i],sigma_q = samples$sigma_q[i], eta = samples$eta[i],
                            nu = samples$nu[i], xi = samples$xi_raw[i])

nsamples = 1000
group1 <- matrix(nrow = nsamples, ncol = 109)
par(cex.main = 1.5,cex.lab = 1.5)
ts.plot(sim$reported_cases, ylim = c(0,2300), ylab = "Reported Cases" , main = "Posterior predictive plot: over-dispersed observations", col = alpha('black', 0.2) )
sim$reported_cases
j=0
for (i in indices) {
  j=j+1
  sim <- simulate_SEIR_forced(T=109, N = 8570000 , beta = samples$beta[i],rho = samples$rho[i], gamma = samples$gamma[i],I0 = samples$i0[i],
                              ...
                              nu = samples$nu[i], xi = samples$xi_raw[i])
  if(i%%300==0){
  lines(sim$reported_cases, col = alpha(i/300, 0.5) )}
...
par(mfrow = c(1,1))
par(cex.main = 1.5,cex.lab = 1.5)
ts.plot(colMeans(group1),ylim = c(0, 2300), ylab = 'New reported cases', main = "Posterior predictive check: over-dispersed observations")
